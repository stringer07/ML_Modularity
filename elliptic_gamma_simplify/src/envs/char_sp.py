...
EVAL_SYMBOLS = {'x', 'y', 'z', 't', 's', 'a0', 'a1', 'a2', 'a3', 'a4', 'a5', 'a6', 'a7', 'a8', 'a9'}
...
def is_valid_expr(s):
    ...
    s = s.replace('(I)', '(1)')
    s = s.replace('(pi)', '(1)')
    s = re.sub(r'(?<![a-z])(f|g|h|q_theta|egamma|Abs|sign|ln|sin|cos|tan|sec|csc|cot|asin|acos|atan|asec|acsc|acot|tanh|sech|csch|coth|asinh|acosh|atanh|asech|acoth|acsch)\(', '(', s)
...
class CharSPEnvironment(object):
    ...
    SYMPY_OPERATORS = {
        ...
        #qtheta functions
        #sp.Function('q_theta'): 'q_theta',
        sp.Function('egamma'): 'egamma',
    }
    ...
    def __init__(self, params):
        ...
        self.variables = OrderedDict({
            ...
            's': sp.Symbol('s', real=True, nonzero=True),  # , positive=True
        })
        ...
        self.functions = OrderedDict({
            ...
            'q_theta': sp.Function('q_theta', real=True, nonzero=True),
            'egamma': sp.Function('egamma', real=True, nonzero=True),
        })
    ...
    def write_infix(self, token, args):
        ...
        if token == 'add':
            ...
            return f'q_theta({args[0]},{args[1]})'
        elif token == 'h':
            return f'h({args[0]},{args[1]},{args[2]})'
        elif token =='egamma':
            return f'egamma({args[0]},{args[1]},{args[2]})'
    ...
    def _sympy_to_prefix(self, op, expr):
        ...
        # 特殊处理 'egamma' 操作符
        if op == 'egamma':
            assert 1 <= n_args <= 3, f"egamma 应该有 1 到 3 个参数，但接收到 {n_args} 个"
            parse_list = [op]  # 只在最初添加一次 'egamma'
            for arg in expr.args:
                parse_list += self.sympy_to_prefix(arg)
            return parse_list
        ...
        # parse children for other operators
        parse_list = []
        for i in range(n_args):
            if i == 0 or i < n_args - 1:
                parse_list.append(op)
